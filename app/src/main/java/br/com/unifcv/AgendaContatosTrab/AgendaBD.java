...
public class AgendaBD extends SQLiteOpenHelper {
    ...
    public List<AgendaInfo> getList(String order){
        List<AgendaInfo> contatos = new ArrayList<>();

        Cursor cursor = getReadableDatabase().rawQuery("SELECT * FROM " + TABELA + " ORDER BY nome " +
            order + ";", null);

        while(cursor.moveToNext()){
            AgendaInfo c = new AgendaInfo();
    ...
    public void inserirContato(AgendaInfo c){
    ...
    public void alteraContato(AgendaInfo c){
        ContentValues values = new ContentValues();
    ...
    public void apagarContato(AgendaInfo c){
